...
def load_assets(user_config: UserConfig) -> Dict[str, List[Asset]]:
    ...
    try:
        for row in BondModel.query.filter_by(user_id=int(current_user.id)).all():
            asset = Bond(
            ...
            for irow in BondScheduleModel.query.filter_by(
                user_id=int(current_user.id), bond_id=row.id
            ).all():
                payment = {
    ...
    for row in DepositCertificateModel.query.filter_by(
        ...
        for irow in DepositCertificateSchedule.query.filter_by(
            user_id=int(current_user.id), cd_id=row.id
        ).all():
            payment = {
                "date": datetime.strptime(irow.date, Config.DATE_FORMAT_STRING),
                "amount": float(irow.amount),
    ...
    for row in Recurrent.query.filter_by(user_id=int(current_user.id)).all():
    ...
    for row in InstrumentModel.query.filter_by(user_id=int(current_user.id)).all():
        qty = float(row.qty)
        ...
        assets[asset.currency].append(asset)
    for row in Account.query.filter_by(user_id=int(current_user.id)).all():
        ...
        assets[asset.currency].append(asset)
    for row in PayableModel.query.filter_by(user_id=int(current_user.id)).all():
        asset = Payable(
        ...
        assets[asset.currency].append(asset)
    for row in PropertyModel.query.filter_by(user_id=int(current_user.id)).all():
        asset = Property(
